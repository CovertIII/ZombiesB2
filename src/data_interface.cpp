...
void prepare_user_list(data_record db){
    ...
    while((result = sqlite3_step(sql)) == SQLITE_ROW){
        ...
        strcpy(node->name, (const char *)sqlite3_column_text(sql, 1));
        max++;
    }
    db->v_max = max-1;
    sqlite3_finalize(sql);
...
void prepare_level_scores(data_record db){
    ...
    for (i = 1; i <= db->max_level; i++) {
        ...
        while((result = sqlite3_step(sql)) == SQLITE_ROW){
            ...
            //Fill up the structure with data
            node->name = (unsigned char*)malloc(sizeof(char)*strlen((const char*)sqlite3_column_text(sql, 3)));
            strcpy((char*)node->name, (const char*)sqlite3_column_text(sql, 3));

            node->slevel_id = (char*)malloc(sizeof(char)*strlen((const char*)sqlite3_column_text(sql, 1)));
            strcpy((char*)node->slevel_id, (const char*)sqlite3_column_text(sql, 1));
